...
const StrategyFlow = memo(function StrategyFlow({ result, t }: { result: CalculationResult; t: Translation }) {
  return (
...
const CalculationLog = memo(function CalculationLog({ result, config, product, safetyGaps, t }: {
      ...
      <div className="p-4 space-y-4 text-sm font-mono text-slate-600 dark:text-slate-300">
        <div>
          ...
          <div className="grid grid-cols-1 gap-1 pl-2 border-l-2 border-indigo-200 dark:border-indigo-700">
            <div>L: ({product.length} × {config.innerArrangement.l}) + {config.innerWallThickness} = <span className="font-bold text-slate-800 dark:text-slate-100">{result.innerBoxDims.length.toFixed(1)}</span></div>
            <div>W: ({product.width} × {config.innerArrangement.w}) + {config.innerWallThickness} = <span className="font-bold text-slate-800 dark:text-slate-100">{result.innerBoxDims.width.toFixed(1)}</span></div>
            <div>H: ({product.height} × {config.innerArrangement.h}) + {config.innerWallThickness} = <span className="font-bold text-slate-800 dark:text-slate-100">{result.innerBoxDims.height.toFixed(1)}</span></div>
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-1 gap-1 pl-2 border-l-2 border-emerald-200 dark:border-emerald-700">
            <div>L: {result.innerBoxDims.length.toFixed(1)} × {config.masterArrangement.l} = <span className="font-bold text-slate-800 dark:text-slate-100">{result.masterPayloadDims.length.toFixed(1)}</span></div>
            <div>W: {result.innerBoxDims.width.toFixed(1)} × {config.masterArrangement.w} = <span className="font-bold text-slate-800 dark:text-slate-100">{result.masterPayloadDims.width.toFixed(1)}</span></div>
            <div>H: {result.innerBoxDims.height.toFixed(1)} × {config.masterArrangement.h} = <span className="font-bold text-slate-800 dark:text-slate-100">{result.masterPayloadDims.height.toFixed(1)}</span></div>
          </div>
        </div>
        <div>
...
const BoxInfoCard = memo(function BoxInfoCard({ result, t }: { result: CalculationResult; t: Translation }) {
  return (
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-100 dark:border-slate-600">
            ...
            <div className="text-xl font-bold text-slate-800 dark:text-slate-100">{result.box.length} x {result.box.width} x {result.box.height} <span className="text-sm font-normal text-slate-500 dark:text-slate-400">mm</span></div>
          </div>
          <div className="p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-100 dark:border-slate-600">
            ...
            <div className="text-xl font-bold text-slate-800 dark:text-slate-100">+{result.gapL.toFixed(0)} / +{result.gapW.toFixed(0)} / +{result.gapH.toFixed(0)} <span className="text-sm font-normal text-slate-500 dark:text-slate-400">mm</span></div>
          </div>
          <div className="p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-100 dark:border-slate-600">
...
const VisualizationCard = memo(function VisualizationCard({ result, t }: { result: CalculationResult; t: Translation }) {
  return (
...
const EfficiencyTooltip = memo(function EfficiencyTooltip({ payloadVol, boxVol, calcEfficiency, t }: {
  payloadVol: number;
  boxVol: number;
...
const AIAnalysisCard = memo(function AIAnalysisCard({ aiAnalysis, loadingAI, payloadVol, boxVol, calcEfficiency, t }: {
  aiAnalysis: AIAnalysis | null;
  loadingAI: boolean;
  payloadVol: number;
  boxVol: number;
          ...
          <div className="space-y-4">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="w-full md:w-auto bg-slate-50 dark:bg-slate-700 p-4 rounded-lg border border-slate-100 dark:border-slate-600 flex flex-col justify-center items-center min-w-[100px] relative cursor-help transition-colors hover:bg-slate-100 dark:hover:bg-slate-600 hover:border-slate-300 dark:hover:border-slate-500 z-10"
                ...
                {showEffTooltip && (
                  <EfficiencyTooltip payloadVol={payloadVol} boxVol={boxVol} calcEfficiency={calcEfficiency} t={t} />
                )}
              </div>
            </div>
            <div className="mt-4">
...
export const ResultDisplay: React.FC<Props> = ({ result, aiAnalysis, loadingAI, t, config, product, safetyGaps = { l: 3, w: 3, h: 2 } }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <StrategyFlow result={result} t={t} />
      <CalculationLog result={result} config={config} product={product} safetyGaps={safetyGaps} t={t} />
      <BoxInfoCard result={result} t={t} />
      <VisualizationCard result={result} t={t} />
      <AIAnalysisCard aiAnalysis={aiAnalysis} loadingAI={loadingAI} payloadVol={payloadVol} boxVol={boxVol} calcEfficiency={calcEfficiency} t={t} />
    </div>
  );
};
